  ...
  return (
    <section className="mt-12 sm:mt-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {cards.map((c) => (
          <Link key={c.id} to={`/news/${c.id}`} className="group block bg-surface rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            <div className="h-40 overflow-hidden">
              <img src={c.image} alt={c.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
            </div>
            <div className="p-4">
              <p className="text-xs text-muted mb-1">{c.category} · {c.readTime}</p>
              <h4 className="text-sm font-semibold text-navy leading-snug line-clamp-2 group-hover:text-teal transition-colors">{c.title}</h4>
            </div>
          </Link>
        ))}
      </div>
